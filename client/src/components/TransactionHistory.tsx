...
export function TransactionHistory({ open, onClose }: TransactionHistoryProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-md bg-card border-border/40" hideDefaultClose>
        <DialogHeader>
        ...
        <ScrollArea className="h-[400px] pr-4">
          {transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-muted-foreground text-xs mt-1">Your swap history will appear here</p>
            </div>
          ) : (
            <div className="space-y-3">
              {transactions.map((tx) => (
                <div 
                  key={tx.id} 
                  className="bg-gradient-to-br from-muted/50 to-muted/30 rounded-xl p-4 border border-border/40 hover:border-primary/40 transition-all"
                >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm font-semibold truncate">{parseFloat(tx.fromAmount).toFixed(4)} {tx.fromToken.symbol}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm font-semibold truncate">{parseFloat(tx.toAmount).toFixed(4)} {tx.toToken.symbol}</span>
                      </div>
                    </div>
                  </div>
                </div>
